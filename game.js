...
function start(){
    bear = new Bear();
    lastStingTime = Number(new Date());
    addBees = document.getElementById("addBee");
    addBees.addEventListener("click", addBee, false);
    document.addEventListener("keydown", moveBear, false);
    speedInput = document.getElementById("speedBear");
    speedInput.addEventListener("change", setSpeed, false)
    bees = new Array();
    numBee = document.getElementById("nbBees");
    numBee.addEventListener("change", makeBees, false);
    //create bees
    makeBees();
    updateBees()
}
function setSpeed(){
    bear.dBear = speedInput.value;
}
...
function addBee(){
    let newBee = new Bee(bees.length);
    newBee.display();
    bees.push(newBee);
}
...
function moveBees() {
 ...
 function updateBees() { // update loop for game
    ...
    let stings = Number(hits.innerHTML);
    if(stings <= 1000){
...
function isHit(defender, offender) {
    if (overlap(defender, offender)) { //check if the two image overlap
        ...
        let newStingTime = Number(new Date());
